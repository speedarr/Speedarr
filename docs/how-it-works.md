...
If a download client stops answering, Speedarr assumes it is still running under the last limit it gave it, and holds that amount out of the pool: the clients it can still reach share what's left, so nothing gets handed out twice. The moment the client answers again it's back in the normal split. A client that hasn't answered at all since Speedarr started holds out its configured share instead, since there's no last limit to go on. The hold lasts until the client answers — there's no timeout on it, so a client you've retired for good is worth removing from Settings.
